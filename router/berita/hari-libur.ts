...
function getDayDifference(date1: string, date2: string): number {
  ...
  if (d2 < d1) d2 = new Date(currentYear + 1, month2 - 1, day2)
...
function isHoliday(event: string): boolean {
  const holidayKeywords = ["libur", "natal", "nyepi", "waisak", "idul", "imlek", "kenaikan", "maulid", "isra", "tahun baru", "cuti bersama"]
  return holidayKeywords.some((k) => event.toLowerCase().includes(k))
}

function isNationalDay(event: string): boolean {
  const nationalKeywords = ["nasional", "indonesia", "nusantara", "proklamasi", "kemerdekaan", "pancasila", "kebangkitan", "pahlawan", "kesaktian", "sumpah pemuda"]
  return nationalKeywords.some((k) => event.toLowerCase().includes(k))
}
...
async function scrapeAndCombineEvents() {
  ...
  function addEvent(date: string, event: string, source: string) {
    ...
    if (!events.has(key) || source === "web") {
      events.set(key, { date, event, source, standardName })
      if (isHoliday(event)) holidays.set(key, { date, event, source })
      if (isNationalDay(event)) nationalDays.set(key, { date, event, source })
    }
  }
  ...
  try {
    ...
    $("article ul").each((_, monthElement) => {
      ...
      $(monthElement).find("table tbody tr").each((_, row) => {
        ...
        if (formattedDate !== "00-00" && monthNumber !== "00") {
          if (event.includes(" dan ")) {
            event.split(" dan ").forEach((e) => addEvent(formattedDate, e.trim(), "web"))
          } else {
    ...
    // fallback ke wikipedia data
  }

  wikipediaData.events.forEach((e) => addEvent(e.date, e.event, "wikipedia"))

  const today = getTodayDate()

  function toSortedArray(map: Map<string, EventData>): EventData[] {
  ...
  function getUpcoming(arr: EventData[], count = 5) {
    return arr
      .map((e) => ({ ...e, daysUntil: getDayDifference(today, e.date) }))
      .filter((e) => e.daysUntil >= 0)
      .sort((a, b) => a.daysUntil - b.daysUntil)
      .slice(0, count)
  }

  function clean(arr: EventData[]) {
    return arr.map(({ standardName, ...e }) => e)
  }

  const allEvents = toSortedArray(events)
  const allHolidays = toSortedArray(holidays)
  const allNationalDays = toSortedArray(nationalDays)

  return {
    hari_ini: {
      tanggal: today,
      events: clean(allEvents.filter((e) => e.date === today)),
    },
    mendatang: {
      event_nasional: clean(getUpcoming(allNationalDays)),
      hari_libur: clean(getUpcoming(allHolidays)),
    },
    data: {
      hari_libur: clean(allHolidays),
      hari_nasional: clean(allNationalDays),
      semua_event: clean(allEvents),
    },
    statistik: {
...
export default async function hariLiburHandler(req: Request, res: Response) {
  try {
    const data = await scrapeAndCombineEvents()

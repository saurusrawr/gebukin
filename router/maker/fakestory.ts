...
// Fungsi helper untuk wrap text (sama seperti di kode bot kamu)
...
export default async function canvasHandler(req: Request, res: Response) {
    ...
    if (!text) {
        return res.status(400).json({
            status: false,
            message: "Parameter 'text' diperlukan. Format: username|caption"
        });
    }
    ...
    try {
        ...
        if (!username || !caption) {
            return res.status(400).json({
                status: false,
                message: "Format salah. Gunakan format: username|caption"
            });
        }
        ...
        // 5. Draw Profile Picture (Circular)
        const ppX = 40;
        const ppY = 250;
        const ppSize = 70;
        ...
        // 7. Draw Caption (Centered & Wrapped)
